...
@section('content')
<div class="relative w-full h-48 bg-cover bg-center" style="background-image: url('{{ $child->cover_url }}')">
...
<div class="p-8 mb-12">
    ...
    <div class="flex mt-4 gap-4 custom-scrollbar">
        @forelse($child->episodes as $episode)
            <a href="{{ route('children.episode', ['child' => $child , 'episode' => $episode]) }}" class="w-40 h-48 text-white rounded-lg relative overflow-hidden group flex-shrink-0 transition-all duration-300 ease-in-out shadow-lg hover:shadow-xl transform group-hover:scale-105 group-hover:rotate-2 group-hover:translate-y-2">
