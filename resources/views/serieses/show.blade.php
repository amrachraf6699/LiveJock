...
@section('content')
<div class="relative w-full h-48 bg-cover bg-center" style="background-image: url('{{ $series->cover_url }}')">
    <div class="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center text-white p-4">
        <h1 class="text-3xl font-bold">{{ $series->name }}</h1>
    </div>
</div>
...
<div class="p-8 mb-12">
    ...
    <div class="flex mt-4 gap-4">
        @forelse($series->episodes as $episode)
            <a href="{{ route('serieses.episode', ['series' => $series , 'episode' => $episode]) }}" class="w-40 h-48 text-white rounded-lg relative overflow-hidden group flex-shrink-0 transition-all duration-300 ease-in-out shadow-lg hover:shadow-xl transform group-hover:scale-105 group-hover:rotate-2 group-hover:translate-y-2">
